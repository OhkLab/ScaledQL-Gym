...
class SACAgent(Agent):
    def __init__(self, memory_size, *args, **kwargs):
        ...
        self.actor_net = ActorNet(
            ...
            output_dims=self.action_dim,
            name="actor",
            chkpt_dir=self.chkpt_dir,
            hidden_size=self.hidden_size,
        )
        self.critic1_net = CriticNet(
            input_dims=self.input_dims,
        ...
        self.critic1_net_target = CriticNet(
            input_dims=self.input_dims,
            action_dim=self.action_dim,
            name="critic1_target",
            chkpt_dir=self.chkpt_dir,
        ...
        self.critic2_net_target = CriticNet(
            input_dims=self.input_dims,
            action_dim=self.action_dim,
            name="critic2_target",
            chkpt_dir=self.chkpt_dir,
        ...
        self.critic_optim = optim.Adam(
            list(self.critic1_net.parameters()) + list(self.critic2_net.parameters()),
            self.lr_critic,
        )
        ...
        self.cql_alpha_optimizer = optim.Adam(
            params=[self.cql_log_alpha], lr=self.lr_critic
        )
    ...
    def update_parameters(self):
        ...
        with torch.no_grad():
            ...
            next_q1_values_target = self.critic1_net_target(
                next_state_batch, next_action
            )
            next_q2_values_target = self.critic2_net_target(
                next_state_batch, next_action
            )
            next_q_values_target = (
                torch.min(next_q1_values_target, next_q2_values_target)
                - self.alpha * next_log_pi
            )
            next_q_values = (
                reward_batch + mask_batch * self.gamma * next_q_values_target
            )
...
class ScaledQLAgent(Agent):
    def __init__(
        ...
        **kwargs,
        ...
        self.critic1_net = ScaledCriticNet(
            input_dims=self.input_dims,
            action_dim=self.action_dim,
            name="critic1",
            chkpt_dir=self.chkpt_dir,
        ...
        self.critic2_net = ScaledCriticNet(
            input_dims=self.input_dims,
            action_dim=self.action_dim,
            name="critic2",
            chkpt_dir=self.chkpt_dir,
        ...
        self.critic1_net_target = ScaledCriticNet(
            input_dims=self.input_dims,
            action_dim=self.action_dim,
            name="critic1_target",
            chkpt_dir=self.chkpt_dir,
        ...
        self.critic2_net_target = ScaledCriticNet(
            input_dims=self.input_dims,
            action_dim=self.action_dim,
            name="critic2_target",
            chkpt_dir=self.chkpt_dir,
    ...
    def update_parameters(self, data):
        ...
        if self.auto_alpha:
            alpha_loss = -(
                self.log_alpha.exp()
                * (log_pi.cpu() + self.target_entropy).detach().cpu()
            ).mean()
            self.alpha_optim.zero_grad()
            alpha_loss.backward()
            self.alpha_optim.step()
        ...
        # CQL
        random_actions = (
            torch.FloatTensor(q1_values.shape[0] * 1, actions.shape[-1])
            .uniform_(-1, 1)
            .to(self.device)
        )
        ...
        temp_states = (
            states.unsqueeze(1)
            .repeat(1, num_repeat, 1, 1, 1)
            .view(
                states.shape[0] * num_repeat,
                states.shape[-3],
                states.shape[-2],
                states.shape[-1],
            )
        ...
        temp_next_states = (
            ...
            .repeat(1, num_repeat, 1, 1, 1)
            .view(
                next_states.shape[0] * num_repeat,
                next_states.shape[-3],
                next_states.shape[-2],
                next_states.shape[-1],
            )
        ...
        assert cat_q1.shape == (
            ...
            3 * num_repeat,
            1,
        ), f"cat_q1 instead has shape: {cat_q1.shape}"
        assert cat_q2.shape == (
            states.shape[0],
            3 * num_repeat,
            1,
        ), f"cat_q2 instead has shape: {cat_q2.shape}"
