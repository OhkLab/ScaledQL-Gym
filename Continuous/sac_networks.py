...
from modified_resnet import CustomCNN
from utils import hidden_init


class CriticNet(nn.Module):
    def __init__(self, input_dims, action_dim, name, chkpt_dir, hidden_size=256):
        super().__init__()
        ...
        self.fc_input_dims = int(np.prod(self.encoder.encoder_output_dims))
        self.fc1 = nn.Linear(self.fc_input_dims + action_dim, hidden_size)
...
class ActorNet(nn.Module):
    def __init__(
        self, input_dims, output_dims, name, chkpt_dir, hidden_size=256, epsilon=1e-6
    ):
        super().__init__()
    ...
    def sample(self, state):
        ...
        x_t = dist.rsample()
        action = x_t * torch.tensor([2.0, 1.0, 1.0], device=self.device).repeat(
            x_t.shape[0], 1
        ) + torch.tensor([-1.0, 0.0, 0.0], device=self.device).repeat(x_t.shape[0], 1)
        log_prob = dist.log_prob(x_t)
        log_prob -= torch.log((1 - action.pow(2)) + self.epsilon)
        log_prob = log_prob.sum(1, keepdim=True)
        action_eval = alpha / (alpha + beta)
        action_eval = action_eval * torch.tensor(
            [2.0, 1.0, 1.0], device=self.device
        ).repeat(x_t.shape[0], 1) + torch.tensor(
            [-1.0, 0.0, 0.0], device=self.device
        ).repeat(x_t.shape[0], 1)
