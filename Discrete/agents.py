...
class DQNAgent(Agent):
    ...
    def choose_action(self, observation):
        if np.random.random() > self.epsilon:
            state = torch.tensor(np.array([observation]), dtype=torch.float).to(
                self.q_eval.device
            )
...
class ScaledQAgent(Agent):
    def __init__(
        self, n_norm_groups, dataloader, alpha, v_min, v_max, atoms, *args, **kwargs
    ):
    ...
    def learn(self):
        ...
        for batch, data in enumerate(self.dataloader):
            ...
            states, actions, rewards, states_, dones = (
                data[0].to(self.q_eval.device),
                data[1].to(self.q_eval.device),
                data[2].to(self.q_eval.device),
                data[3].to(self.q_eval.device),
...
class CategoricalScaledQAgent(Agent):
    def __init__(
        ...
        if dataloader is not None:
            self.dataloader = dataloader
        self.alpha = alpha
        self.q_eval = CategoricalScaledQNetwork(
            self.lr,
            self.eps,
            self.n_norm_gropus,
            self.n_actions,
        ...
        self.q_next = CategoricalScaledQNetwork(
            self.lr,
            self.eps,
            self.n_norm_gropus,
            self.n_actions,
    ...
    def learn(self):
        ...
        for batch, data in enumerate(self.dataloader):
            ...
            offset = (
                torch.arange(batch_size).view(-1, 1).to(self.q_eval.device) * self.atoms
            )
