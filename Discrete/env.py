...
class RepeatActionAndMaxFrame(gym.Wrapper):
    def __init__(
        self, env=None, repeat=4, clip_reward=False, no_ops=30, fire_first=False
    ):
    ...
    def step(self, action):
        ...
        for i in range(self.repeat):
            obs, reward, done, info, info2 = self.env.step(action)
            if self.clip_reward:
        ...
        return max_frame, t_reward, done, info, info2
...
class PreprocesesFrame(gym.ObservationWrapper):
    def __init__(self, shape, env=None):
        super(PreprocesesFrame, self).__init__(env)
...
def make_env(
    ...
    env = PreprocesesFrame(shape, env)
