...
def main(cfg: DictConfig):
    ...
    env = make_env(cfg.env.env_name, no_ops=30)
    best_score = -np.inf
    ...
    if cfg.util.make_video:
        ...
        env = wrappers.Monitor(
            env, video_dir, video_callable=lambda x: x % 10 == 0, force=True
        )
    ...
    for i in range(cfg.util.n_games):
        ...
        while not done:
            ...
            if not cfg.util.load_checkpoint:
                ...
                agent.learn()
            if cfg.util.save_memory and n_steps % 100 == 0:
                transition = [observation, action, reward, observation_, int(done)]
                np.save(
                    f"{memory_dir}/{n_save_data}.npy",
                    np.array(transition, dtype=object),
                )
                n_save_data += 1
                if n_save_data == cfg.util.total_save_data:
                    exit()
